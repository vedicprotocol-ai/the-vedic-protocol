...
\restrict EmBnXWIDAeIrL0b23gBu8JbW3cKmwjaw0IVEtMhqHUKXedPGtaUAbcJeexjwK7w
...
CREATE TRIGGER trg_touch_appointments BEFORE UPDATE ON public.appointments FOR EACH ROW EXECUTE FUNCTION public.touch_updated();
...
CREATE TRIGGER trg_touch_availability_slots BEFORE UPDATE ON public.availability_slots FOR EACH ROW EXECUTE FUNCTION public.touch_updated();
...
CREATE TRIGGER trg_touch_blog_posts BEFORE UPDATE ON public.blog_posts FOR EACH ROW EXECUTE FUNCTION public.touch_updated();
...
CREATE TRIGGER trg_touch_cart_items BEFORE UPDATE ON public.cart_items FOR EACH ROW EXECUTE FUNCTION public.touch_updated();
...
CREATE TRIGGER trg_touch_contact_submissions BEFORE UPDATE ON public.contact_submissions FOR EACH ROW EXECUTE FUNCTION public.touch_updated();
...
CREATE TRIGGER trg_touch_coupon_usage BEFORE UPDATE ON public.coupon_usage FOR EACH ROW EXECUTE FUNCTION public.touch_updated();
...
CREATE TRIGGER trg_touch_coupons BEFORE UPDATE ON public.coupons FOR EACH ROW EXECUTE FUNCTION public.touch_updated();
...
CREATE TRIGGER trg_touch_customers BEFORE UPDATE ON public.customers FOR EACH ROW EXECUTE FUNCTION public.touch_updated();
...
CREATE TRIGGER trg_touch_doctors BEFORE UPDATE ON public.doctors FOR EACH ROW EXECUTE FUNCTION public.touch_updated();
...
CREATE TRIGGER trg_touch_influencers BEFORE UPDATE ON public.influencers FOR EACH ROW EXECUTE FUNCTION public.touch_updated();
...
CREATE TRIGGER trg_touch_loyalty_points BEFORE UPDATE ON public.loyalty_points FOR EACH ROW EXECUTE FUNCTION public.touch_updated();
...
CREATE TRIGGER trg_touch_newsletter_subscribers BEFORE UPDATE ON public.newsletter_subscribers FOR EACH ROW EXECUTE FUNCTION public.touch_updated();
...
CREATE TRIGGER trg_touch_orders BEFORE UPDATE ON public.orders FOR EACH ROW EXECUTE FUNCTION public.touch_updated();
...
CREATE TRIGGER trg_touch_products BEFORE UPDATE ON public.products FOR EACH ROW EXECUTE FUNCTION public.touch_updated();
...
CREATE TRIGGER trg_touch_protocol_signups BEFORE UPDATE ON public.protocol_signups FOR EACH ROW EXECUTE FUNCTION public.touch_updated();
...
CREATE TRIGGER trg_touch_reviews BEFORE UPDATE ON public.reviews FOR EACH ROW EXECUTE FUNCTION public.touch_updated();
...
CREATE POLICY "Admin full access" ON public.doctors USING ((lower(( SELECT customers.role
...
CREATE POLICY "Admins can read all profiles" ON public.customers FOR SELECT USING ((EXISTS ( SELECT 1
...
CREATE POLICY "Users can read own profile" ON public.customers FOR SELECT USING ((auth.uid() = id));
...
CREATE POLICY appts_self_rw ON public.appointments USING (((customer_id = auth.uid()) OR public.is_admin())) WITH CHECK (((customer_id = auth.uid()) OR public.is_admin()));
...
CREATE POLICY cart_self_rw ON public.cart_items USING (((customer_id = auth.uid()) OR public.is_admin())) WITH CHECK (((customer_id = auth.uid()) OR public.is_admin()));
...
CREATE POLICY contact_admin ON public.contact_submissions FOR SELECT USING (public.is_admin());
...
CREATE POLICY coupon_usage_read ON public.coupon_usage FOR SELECT USING (((customer_id = auth.uid()) OR public.is_admin()));
...
CREATE POLICY coupon_usage_write ON public.coupon_usage FOR INSERT WITH CHECK (true);
...
CREATE POLICY coupons_admin ON public.coupons USING (public.is_admin()) WITH CHECK (public.is_admin());
...
CREATE POLICY customers_admin_all ON public.customers USING (public.is_admin()) WITH CHECK (public.is_admin());
...
CREATE POLICY customers_self_insert ON public.customers FOR INSERT WITH CHECK ((id = auth.uid()));
...
CREATE POLICY customers_self_read ON public.customers FOR SELECT USING (((id = auth.uid()) OR public.is_admin()));
...
CREATE POLICY customers_self_update ON public.customers FOR UPDATE USING (((id = auth.uid()) OR public.is_admin()));
...
CREATE POLICY doctors_admin_write ON public.doctors USING (public.is_admin()) WITH CHECK (public.is_admin());
...
CREATE POLICY influencers_admin ON public.influencers USING (public.is_admin()) WITH CHECK (public.is_admin());
...
CREATE POLICY influencers_self_read ON public.influencers FOR SELECT USING (((customer_id = auth.uid()) OR (user_id = auth.uid()) OR public.is_admin()));
...
CREATE POLICY loyalty_admin_all ON public.loyalty_points FOR UPDATE USING (public.is_admin());
...
CREATE POLICY loyalty_self_ins ON public.loyalty_points FOR INSERT WITH CHECK (((customer_id = auth.uid()) OR public.is_admin()));
...
CREATE POLICY loyalty_self_read ON public.loyalty_points FOR SELECT USING (((customer_id = auth.uid()) OR public.is_admin()));
...
CREATE POLICY orders_self_rw ON public.orders USING (((customer_id = auth.uid()) OR public.is_admin())) WITH CHECK (((customer_id = auth.uid()) OR public.is_admin()));
...
CREATE POLICY products_admin_write ON public.products USING (public.is_admin()) WITH CHECK (public.is_admin());
...
--
-- Name: protocol_signups; Type: ROW SECURITY; Schema: public; Owner: -
--
...
CREATE POLICY reviews_self_write ON public.reviews USING (((customer_id = auth.uid()) OR public.is_admin())) WITH CHECK (((customer_id = auth.uid()) OR public.is_admin()));
...
CREATE POLICY signup_admin ON public.protocol_signups FOR SELECT USING (public.is_admin());
...
CREATE POLICY signup_insert ON public.protocol_signups FOR INSERT WITH CHECK (true);
...
CREATE POLICY slots_admin_write ON public.availability_slots USING (public.is_admin()) WITH CHECK (public.is_admin());
...
CREATE POLICY slots_auth_update ON public.availability_slots FOR UPDATE USING ((auth.uid() IS NOT NULL)) WITH CHECK ((auth.uid() IS NOT NULL));
...
CREATE POLICY slots_public_read ON public.availability_slots FOR SELECT USING (true);
...
\unrestrict EmBnXWIDAeIrL0b23gBu8JbW3cKmwjaw0IVEtMhqHUKXedPGtaUAbcJeexjwK7w
