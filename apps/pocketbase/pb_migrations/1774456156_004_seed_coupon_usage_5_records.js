...
migrate((app) => {
  ...
  const record0 = new Record(collection);
    ...
    const record0_customer_idLookup = app.findFirstRecordByFilter("customers", "email='priya.sharma@example.com'");
    if (!record0_customer_idLookup) { throw new Error("Lookup failed for customer_id: no record in 'customers' matching \"email='priya.sharma@example.com'\""); }
    record0.set("customer_id", record0_customer_idLookup.id);
    record0.set("purchase_amount", 150);
    record0.set("discount_amount", 30);
    record0.set("influencer_earning", 7.5);
  try {
    app.save(record0);
  } catch (e) {
  ...
  const record1 = new Record(collection);
    ...
    const record1_customer_idLookup = app.findFirstRecordByFilter("customers", "email='aisha.khan@example.com'");
    if (!record1_customer_idLookup) { throw new Error("Lookup failed for customer_id: no record in 'customers' matching \"email='aisha.khan@example.com'\""); }
    record1.set("customer_id", record1_customer_idLookup.id);
    record1.set("purchase_amount", 200);
    record1.set("discount_amount", 40);
  ...
  const record2 = new Record(collection);
    ...
    const record2_customer_idLookup = app.findFirstRecordByFilter("customers", "email='maya.patel@example.com'");
    if (!record2_customer_idLookup) { throw new Error("Lookup failed for customer_id: no record in 'customers' matching \"email='maya.patel@example.com'\""); }
    record2.set("customer_id", record2_customer_idLookup.id);
    record2.set("purchase_amount", 120);
    record2.set("discount_amount", 24);
    record2.set("influencer_earning", 6);
  try {
    app.save(record2);
  } catch (e) {
    if (e.message.includes("Value must be unique")) {
  ...
  const record3 = new Record(collection);
    ...
    const record3_customer_idLookup = app.findFirstRecordByFilter("customers", "email='neha.gupta@example.com'");
    if (!record3_customer_idLookup) { throw new Error("Lookup failed for customer_id: no record in 'customers' matching \"email='neha.gupta@example.com'\""); }
    record3.set("customer_id", record3_customer_idLookup.id);
    record3.set("purchase_amount", 180);
    record3.set("discount_amount", 36);
    record3.set("influencer_earning", 9);
  try {
    app.save(record3);
  } catch (e) {
    if (e.message.includes("Value must be unique")) {
  ...
  const record4 = new Record(collection);
    ...
    const record4_customer_idLookup = app.findFirstRecordByFilter("customers", "email='zara.ahmed@example.com'");
    if (!record4_customer_idLookup) { throw new Error("Lookup failed for customer_id: no record in 'customers' matching \"email='zara.ahmed@example.com'\""); }
    record4.set("customer_id", record4_customer_idLookup.id);
    record4.set("purchase_amount", 95);
    record4.set("discount_amount", 19);
    record4.set("influencer_earning", 4.75);
  try {
    app.save(record4);
  } catch (e) {
    if (e.message.includes("Value must be unique")) {
